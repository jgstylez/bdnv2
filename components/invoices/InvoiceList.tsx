...
interface InvoiceListProps {
  ...
  invoices?: Invoice[]; // Optional: if not provided, uses mock data
  onInvoicesChange?: (invoices: Invoice[]) => void; // Optional: callback for invoice updates
}

// Mock invoices - in production, fetch from API
const getMockInvoices = (entityType: "business" | "nonprofit"): Invoice[] => {
...
export function InvoiceList({ entityType, invoices: externalInvoices, onInvoicesChange }: InvoiceListProps) {
  const router = useRouter();
  const { isMobile, paddingHorizontal } = useResponsive();
  
  const [invoices, setInvoices] = useState<Invoice[]>(externalInvoices || getMockInvoices(entityType));
  const [searchQuery, setSearchQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState<InvoiceStatus | "all">("all");
  const [typeFilter, setTypeFilter] = useState<"all" | "one-time" | "recurring">("all");

  // Update invoices if external invoices change
  React.useEffect(() => {
    if (externalInvoices) {
  ...
  return (
    <>
      ...
      <View style={{ marginBottom: spacing.xl }}>
        <TouchableOpacity
          onPress={() => router.push(`/pages/invoices/create?type=${getRouteType()}`)}
          style={{
        ...
        <View style={{ flexDirection: isMobile ? "column" : "row", gap: spacing.md }}>
          <TouchableOpacity
            onPress={() => router.push(getTemplatesRoute())}
            style={{
              flex: 1,
              ...
              <AdminDataCard
                ...
                actions={[
                  ...
                  },
                  {
                    ...
                    icon: "edit",
                    variant: "secondary",
                    onPress: () => router.push(`/pages/invoices/create?id=${invoice.id}&type=${getRouteType()}`),
                  },
                  {
