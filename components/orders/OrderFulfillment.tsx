/**
 * Shared Order Fulfillment Component
 * 
 * Displays orders for both businesses and nonprofits
 * Uses entityType prop to determine context, routing, and order type filters
 */
...
interface OrderFulfillmentProps {
  ...
  orders?: Order[]; // Optional: if not provided, uses mock data
  onOrdersChange?: (orders: Order[]) => void; // Optional: callback for order updates
}
...
const getMockOrders = (entityType: "business" | "nonprofit"): Order[] => {
...
// Get order type filter options based on entity type
const getOrderTypeOptions = (entityType: "business" | "nonprofit") => {
  const baseOptions = [
...
export function OrderFulfillment({ entityType, orders: externalOrders, onOrdersChange }: OrderFulfillmentProps) {
  ...
  const isLoading = entityType === "business" 
    ? businessContext.isLoading 
    : nonprofitContext.isLoading;
  
  const entityId = selectedEntity?.id;
  const entityName = selectedEntity?.name || "";

  const [orders, setOrders] = useState<Order[]>(externalOrders || getMockOrders(entityType));
  ...
  // Update orders if external orders change
  useEffect(() => {
    if (externalOrders) {
  ...
  // Load orders for selected entity
  useEffect(() => {
    if (selectedEntity) {
      const entityOrders = orders.filter(order => order.entityId === entityId);
  ...
  useEffect(() => {
    ...
    // Filter by entity
    if (selectedEntity) {
      filtered = filtered.filter((order) => order.entityId === entityId);
    }

    setFilteredOrders(filtered);
    setCurrentPage(1);
  }, [searchQuery, selectedStatus, selectedFulfillmentStatus, selectedPaymentStatus, selectedOrderType, orders, selectedEntity, entityId]);
  ...
  // Check if tracking button should be shown for an order
  const shouldShowTrackingButton = (order: Order) => {
    if (order.paymentStatus !== "completed" || order.shippingInfo?.trackingNumber) {
      return false;
    }
    // Nonprofits exclude donations from tracking
    if (entityType === "nonprofit" && order.orderType === "donation") {
      return false;
    }
    return true;
  };
  ...
  return (
    <>
      ...
      <HeroSection
        ...
        subtitle={`Manage ${entityType === "nonprofit" ? "orders and donations" : "and fulfill orders"} for ${entityName}`}
      />
      ...
      >
        ...
        >
          ...
          <FilterDropdown
            label="Type"
            options={getOrderTypeOptions(entityType)}
          ...
          paginatedOrders.map((order) => (
            ...
            >
              ...
              <View style={{ flexDirection: "row", gap: spacing.sm }}>
                ...
                {shouldShowTrackingButton(order) && (
                  <TouchableOpacity
                    onPress={(e) => {
                      e.stopPropagation();
