...
export function KeyFeatures({ isMobile }: KeyFeaturesProps) {
  ...
  return (
    <View className="mb-8">
      <Text className="text-xl font-bold text-dark-foreground mb-4">Explore Features</Text>
      <View className={cn('flex-row flex-wrap', { 'gap-3': isMobile, 'gap-4': !isMobile })}>
        {features.map((feature, index) => (
          ...
          >
            ...
            <Text className="text-base font-semibold text-dark-foreground mb-1">{feature.title}</Text>
            <Text className="text-sm text-dark-muted-foreground">{feature.description}</Text>
          </TouchableOpacity>
        ))}
      </View>
    </View>
