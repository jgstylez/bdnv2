 ...
 * 
 * Allows businesses to view, filter, and manage service bookings
 */
...
export default function BookingsManagement() {
  ...
  const [filteredBookings, setFilteredBookings] = useState<ServiceBooking[]>(mockBookings);
  ...
  // Load bookings for selected business
  useEffect(() => {
    if (selectedBusiness) {
      const businessBookings = mockBookings.filter(booking => booking.merchantId === selectedBusiness.id);
  ...
  useEffect(() => {
    ...
    // Filter by business
    if (selectedBusiness) {
      filtered = filtered.filter((booking) => booking.merchantId === selectedBusiness.id);
    }

    setFilteredBookings(filtered);
    setCurrentPage(1);
  }, [searchQuery, selectedStatus, bookings, selectedBusiness]);
